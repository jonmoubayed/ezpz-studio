...
function ProviderIcon({ id }: { id: string }) {
  ...
  return <span className={`credential-icon credential-icon-${id}`} aria-hidden="true">
    {asset ? <img src={`/studio/assets/providers/${asset}.svg`} alt="" />
...
export function CredentialSettings({ live, onSaved, onDefaults, onWorkspace }: {
  ...
  if (!live) return <section className="credential-empty">
    <Monitor size={28} /><h2>Connections in the installed Studio</h2>
    <p>Manage provider connections in the installed Studio. This browser demo uses sample results and does not accept API keys.</p>
