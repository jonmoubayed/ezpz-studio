    ...
    >
      <td className="relative p-0 align-top">
        ...
        <div className="min-w-0">
          <InlineTextInput
            value={value.value}
            aria-label={`Enum value ${value.value || "new value"}`}
      ...
      >
        <td className="relative p-0 align-top">
          ...
          <div className="min-w-0">
            <InlineTextInput
              value={property.key}
              aria-label={`Field name ${property.key || "new property"}`}
...
export const SchemaJsonView = React.memo(function SchemaJsonView({
  ...
  const file = React.useMemo(() => {
    const contents = formatJson(jsonSchema ?? serializeSchema(schema));
    return {
      name: "schema.json",
      contents,
      lang: "json" as const,
